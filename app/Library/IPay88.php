...
class IPay88 {
  ...
  private $payment_request = array(
    ...
    'ResponseURL'  => '',  // (Mandatory) Payment response page.
    'BackendURL'  => '',  // (Mandatory) Payment response page.
  );
  ...
  public function generateSignature($signature_params = array()) {
    ...
    if ($signature_params) {
      ...
      $signature_params['Amount'] = $this->getField('Amount'); //str_replace('.', '', str_replace(',', '', $this->getField('Amount')));
